...
-- Answer: actor_award violates the rule of 1NF -- according to 1NF rule 'every coloumn have a atomic value' but in actor_award table there are multiple -- values in award column . -- for normalize in 1NF we have to creat a new table for award
...
-- Answer 2: film table from sakila databse violates 2nf because of the special feature column on the table violate 1nf and 2nf has a rule that table is in 1nf 
...
-- we can create a another table and make them columns foreign keys and these foreign keys make reference to that film id table by using these steps we can avoid 2 nf .
...
-- Answer: if we saw the customer table in the sakila database we get to know that the column name address_id is linked with store id
-- and both are non key attribute and 3nf stays that table is in 2 nf from and it ensure that all the non key attribute column on the table are not related with each other 
-- (one non key attribute column related to other non key attribute column) so because of that it violates 2 and 3nf
...
-- Answer: in this database "film " table seems like unnormalize form so we discuss the normalizetion process up to 2nf
...
-- Recurcive CTE
-- Use a recursive CTE to generate a hierarchical list of categories and their subcategories from the category table in Sakilaa
...
SELECT c.category_id, c.name AS category_name, c.parent_id AS parent_category_id, ch.depth + 1 AS depth
FROM category c
JOIN CategoryHierarchy ch ON c.parent_id = ch.category_id;

-- CTE with Joins:
-- Create a CTE that combines information from the film and languge tables to display the film title, language -- name, and rental rate
...
-- CTE for Aggregation:
-- Write a query using a CTE to find the total revenue generated by each customer (sum of payments) from the customer and payments tables
with totalcustomer_revenue AS ( SELECT c.customer_id, CONCAT(c.first_name, ' ', c.last_name) AS customer_name, SUM(p.amount) 
AS total_revenue FROM customer c LEFT JOIN payment p ON c.customer_id = p.customer_id GROUP BY c.customer_id, customer_name ) 
SELECT customer_id, customer_name, total_revenue FROM totalcustomer_revenue ORDER BY total_revenue;

-- CTE with Window Functions:
-- Utilize a CTE with a window function to rank films based on their rental duration from the tables
...
select c.*,rental_count FROM customer c join customerrentals on c.customer_id =customerrentals.customer_id 
...
-- In this mavenmovies database there is no any column name as payments method so we use amount in the place of payments method
...
select ap.actor1_id, 
CONCAT(a1.first_name, ' ', a1.last_name) AS 1actor, ap.actor2_id, 
CONCAT(a2.first_name, ' ', a2.last_name) AS 2actor, ap.films_together_count 
FROM ActorPairs ap 
...
-- Implement a recursive CTE to find all employees in the staff table who report to a specific manager, considering the reports to column.
